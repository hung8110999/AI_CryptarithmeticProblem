def is_valid_cryptarithmetic_string(s):
    ...
    def is_valid_expression(expression):
        ...
        for i, char in enumerate(expression):
            ...
            if i < len(expression) - 2:
                if expression[i] in allowed_characters and expression[i + 1] == ')' and expression[i + 2] in allowed_characters:
                    return False
                if expression[i] in allowed_characters and expression[i + 1] == '(' and expression[i + 2] in allowed_characters:
                    return False
    ...
    if all(char not in allowed_operators for char in left_part) or any(char in allowed_operators for char in right_part):
        return False
